...
class MarketService
{
    ...
    public function getusers(): array
    {
        ...
        if (!$idToken) {
            Log::error("MarketService: Impossibile estrarre l'id_token dallo Step di Auth");
            dd([
        ...
        $gatewayUrl = rtrim(config('services.fossr.market_url'), '/');

        $vocabulariesUrl = $gatewayUrl . '/vocabularies';
        $datasetsUrl = $gatewayUrl . '/datasets';
        $toolsServicesUrl = $gatewayUrl . '/tools-services';
        $publicationsUrl = $gatewayUrl . '/publications';
        $itemsCategoriesUrl = $gatewayUrl . '/items-categories';
        $itemSearchUrl = $gatewayUrl . '/item-search?q=';

        $usersUrl = $gatewayUrl . '/users';
        ...
        if ($usersResponse->failed()) {
            Log::error('MarketService: Fallito recupero progetti dal gateway');
            dd([
                'ERRORE' => 'Fallito recupero progetti',
                'STATUS' => $usersResponse->status(),
                'BODY' => $usersResponse->json() ?? $usersResponse->body()
            ]);
        }

        $progetti = $usersResponse->json();
        ...
        $outputData = [
            ...
            'total_users' => is_array($progetti) ? count($progetti) : 0,
            'users' => $progetti
        ];

        // Definizione dei nomi file
        $timestampName = 'fossr_users_vms_' . now()->format('Ymd_His') . '.json';
        $latestName = 'fossr_users_vms_latest.json';
        ...
        Log::info("MarketService: Dati salvati con successo in storage: {$timestampName}");
